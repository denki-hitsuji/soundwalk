...
> **2026-07-29 追記**: Hugo側のアーカイブページ要件を受け、期間フィルタを撤廃（過去・今後すべて返す仕様に変更）。詳細は4章-7参照。
...
ユーザー提示の形との差分（2026-07-28 確定、期間フィルタは2026-07-29に変更）:
...
| 期間フィルタ | **2026-07-29に変更**: v1では「今後のライブのみ」（`event_date >= 当日`）としていたが、Hugo側にアーカイブページ（過去ライブ一覧）要件が発生したため撤廃。**過去・今後を問わずすべて返す**。`events` は `date` 昇順で返るので、呼び出し側で `date >= 今日` / `date < 今日` を比較して今後・過去に振り分ける（`GET /api/public/bands/{slug}/lives` のリクエストは1回のみで済む）。 |
...
7. **期間フィルタの撤廃** — 2026-07-29 追加・確定
   ザ・ホリデイズ公式サイト（Hugo）側で、今後のライブページに加えて過去ライブのアーカイブページが必要になった。当初の「今後のライブのみ返す」設計ではアーカイブページを実現できないため、`getPublicActLivesDb` から `event_date >= 当日` のフィルタを撤廃し、**過去・今後を問わずすべてのライブ情報を返す**ように変更した。Hugo側は取得した `events` を `date` で今後/過去に振り分けて、それぞれ別ページに描画する想定。
