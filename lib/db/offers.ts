...
import { Offer, OfferWithMusician } from "../utils/offers";

// イベントごとの応募一覧（店舗側で使用）
export async function getOffersForEventDb(
  eventId: string
 ...
 */
export async function applyToEventDb(eventId: string, message?: string): Promise<Offer> {
  const supabase = await createSupabaseServerClient();
  const user = await getCurrentUser();
  if(!user) {
 ...
 */
export async function getMyOffersDb(): Promise<Offer[]> {
  const supabase = await createSupabaseServerClient();
  const user = await getCurrentUser();
  if(!user) {
    throw new Error('ログインが必要です');
 ...
 */
export async function acceptOfferDb(offerId: string) {
  const supabase = await createSupabaseServerClient();
